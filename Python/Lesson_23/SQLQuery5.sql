...
SELECT [First name], [Last name], [Grant]
FROM Students
WHERE [Grant] <> ALL (SELECT [Grant]
FROM Students
WHERE Group_id = 2 AND [Grant] IS NOT NULL); */

-- Запрос 22

/* SELECT *
FROM Students INNER JOIN [Group]
ON [Group].Group_id = Students.Group_id; */

-- Запрос 23

/* SELECT [First_name], [Last_name], Department, [Subject]
FROM Teachers JOIN Department ON Teachers.Department_id = Department.Department_id
JOIN Teachers_subjects ON Teachers.Teacher_id = Teachers_subjects.Teacher_id
JOIN Subjects ON Teachers_subjects.Subject_id = Subjects.Subject_id
ORDER BY [Subject]; */
