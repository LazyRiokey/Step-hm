...
function sortShopList(shopObj) {
    shopObj.sort(
        function(propA, propB) {
            ...
            // Лучше использовать три ветки, как мы разбирали на практике, – так как в случае равенства двух сравниваемых элементов сортируемого массива они могут преставляться и меняться местами. Сам массив от этого не изменится, но будут выполнены дополнительные лишние действия, которых можно избежать.
        });
}
...
function letCompare(newObj, shopObj = shoppingList) {
    // Не очень понятное имя функции letCompare – переводится как Давайте сравним? Этой функции более подойдёт имя addProduct – добавить покупку.

    let indexPN = shopObj.findIndex(item => item.productName == newObj.productName);
    // Можно использовать метод find, который в случае удачного поиска вернул бы объект покупки, с которым можно было бы работать дальше без необходимости обращаться по индексу к элементу массиву списка покупок.
    ...
    if (indexPN != -1) {
        ...
        // Записывайте фигурные скобки блоков в конструкциях условий либо на отдельных строках, либо с пробелами по краям изнутри, как это сделано на строке 108. Но запись на отдельных строках выглядит гораздо нагляднее и читабельнее.
...
function filterToBuy(req2Buy = Boolean(), shopObj = shoppingList) {
    // Вместо Boolean() можно просто написать false, и не будет лишних действий со стороны интерпретатора по вызову объекта Boolean
...
// Для более наглядного и лёгкого восприятия работы кода стоит логически взаимосвязанные фрагменты кода объединять через использование пустых строк, которыми эти фрагменты отделяются от остальных
